...
class TelnetController:
    ...
    def run_command(self, command, flag):
        ...
        if command.find('rate')!=-1 or command.find('new')!=-1 or len(command.split( ))==2:
            time.sleep(2)
        ...
        if count_ch!=-1:
            ...
            '''self.tn.write(command + '\r\n')
            response_temp=self.tn.read_until('#')'''
            pos=response_temp.find(' # ')
            end=response_temp.find(')')
            response_temp=response_temp[pos+2:end+1]
